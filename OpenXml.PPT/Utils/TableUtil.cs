...
{
    public static D.Table CreateTable(
        SlidePart slidePart,
    ...
    private static D.TableCell NewCell(
        string text, int fontSize = 1100,
        D.TextAlignmentTypeValues alignment = D.TextAlignmentTypeValues.Center)
    {
        // a:tc(TableCell)->a:txbody(TextBody)->a:p(Paragraph)->a:r(Run)->a:t(Text)
        var tableCell = new D.TableCell();
        var textBody = new D.TextBody();
        var paragraph = new D.Paragraph(new D.ParagraphProperties()
        {
            Alignment = alignment,
            FontAlignment = D.TextFontAlignmentValues.Center
        });
        var run = new D.Run();
