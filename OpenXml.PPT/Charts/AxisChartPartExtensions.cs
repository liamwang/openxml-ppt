...
public static class AxisChartPartExtensions
{
    ...
    /// 修改条形图数据
    /// </summary>
    public static void ResetChartSeriesData(
        this ChartPart chartPart, 
        List<ChartSeriesModel> data, 
    ...
    private static LineChartSeries GenerateLineChartSeries(
        ChartSeriesModel model, 
        IEnumerable<string> categories, 
        int index)
    {
        var chartSeries = new LineChartSeries();
        chartSeries.Append(new C.Index() { Val = (uint)index + 2 });
    ...
    private static BarChartSeries GenerateBarChartSeries(
        ChartSeriesModel item, 
        IEnumerable<string> categories, 
        int index)
    {
        var chartSeries = new BarChartSeries();
        chartSeries.Append(new C.Index() { Val = (uint)index });
